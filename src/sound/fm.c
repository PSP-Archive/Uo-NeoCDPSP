...
static s32 *SIN_TABLE[SIN_ENT];

/* envelope output curve table */
#if FM_SEG_SUPPORT
/* attack + decay + SSG upside + OFF */
static s32 ENV_CURVE[3 * EG_ENT + 1];
#else
/* attack + decay + OFF */
static s32 ENV_CURVE[2 * EG_ENT + 1];
#endif
...
#if FM_LFO_SUPPORT
static u32 LFOCnt, LFOIncr;	/* LFO PhaseGenerator */
#endif

/* runtime work */
static s32 out_ch[4];		/* channel output NONE,LEFT,RIGHT or CENTER */
static s32 pg_in1, pg_in2, pg_in3, pg_in4;	/* PG input of SLOTs */

#if 0
static void print_struct(FILE *f,YM2610 *F2610,u8 *data) {
    int i,j;
...
static void dump_mem(FILE *f,YM2610 *F2610,u8 *data,int size) {
    int i;
    fprintf(f,"%04x : ",0);
    for(i=0;i<size;i++) {
...
void dump_struct(YM2610 *F2610,int a) {
	...
	f=fopen("log_load","w");
    fprintf(f,"YM2610:\n");
    dump_mem(f,F2610,(u8*)F2610,sizeof(YM2610));
    fclose(f);
}
...
{
    s32 *carrier = &out_ch[CH->PAN];	/* NONE,LEFT,RIGHT or CENTER */
...
inline void set_ar_ksr(FM_CH * CH, FM_SLOT * SLOT, int v, s32 * ar_table)
{
    SLOT->KSR = 3 - (v >> 6);
...
/* set decay rate */
inline void set_dr(FM_SLOT * SLOT, int v, s32 * dr_table)
{
...
/* set sustain rate */
inline void set_sr(FM_SLOT * SLOT, int v, s32 * dr_table)
{
...
/* set release rate */
inline void set_sl_rr(FM_SLOT * SLOT, int v, s32 * dr_table)
{
...
{
    u32 eg_out1, eg_out2, eg_out3, eg_out4;	/*envelope output */

    /* Phase Generator */
#if FM_LFO_SUPPORT
    s32 pms = lfo_pmd * CH->pms / LFO_RATE;
    if (pms) {
	pg_in1 = (CH->SLOT[SLOT1].Cnt +=
		  CH->SLOT[SLOT1].Incr +
		  (s32) (pms * CH->SLOT[SLOT1].Incr) / PMS_RATE);
	pg_in2 = (CH->SLOT[SLOT2].Cnt +=
		  CH->SLOT[SLOT2].Incr +
		  (s32) (pms * CH->SLOT[SLOT2].Incr) / PMS_RATE);
	pg_in3 = (CH->SLOT[SLOT3].Cnt +=
		  CH->SLOT[SLOT3].Incr +
		  (s32) (pms * CH->SLOT[SLOT3].Incr) / PMS_RATE);
	pg_in4 = (CH->SLOT[SLOT4].Cnt +=
		  CH->SLOT[SLOT4].Incr +
		  (s32) (pms * CH->SLOT[SLOT4].Incr) / PMS_RATE);
    } else
#endif
    {
...
static void init_timetables(FM_ST * ST, u8 * DTTABLE, int ARRATE,
			    int DRRATE)
{
    int i, d;
    double rate;
	...
	for (i = 0; i <= 31; i++) {
	    ...
	    ST->DT_TABLE[d][i] = (s32) rate;
	    ST->DT_TABLE[d + 4][i] = (s32) - rate;
	}
    }
	...
	ST->AR_TABLE[i] = (s32) (rate / ARRATE);
	ST->DR_TABLE[i] = (s32) (rate / DRRATE);
    }
...
static int FMInitTable(void)
{
    ...
    TL_TABLE = (s32 *) malloc(2 * TL_MAX * sizeof(int));
    if (TL_TABLE == 0)
	return 0;
...
static void FMsave_state_channel(FM_CH *CH,int num_ch)
{
    ...
    for(ch=0;ch<num_ch;ch++,CH++) {
	/* channel */
	create_state_register(ST_YM2610_FM, "op1_out1" ,ch+1, &CH->op1_out[0] , 1*sizeof(s32),REG_INT32);
	create_state_register(ST_YM2610_FM, "op1_out2" ,ch+1, &CH->op1_out[1] , 1*sizeof(s32),REG_INT32);
	create_state_register(ST_YM2610_FM, "phasestep"   ,ch+1, &CH->fc , 1*sizeof(u32),REG_u32);
	create_state_register(ST_YM2610_FM, "fn_h"   ,ch+1, &CH->fn_h , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_FM, "kcode"   ,ch+1, &CH->kcode , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_FM, "PAN"   ,ch+1, &CH->PAN , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_FM, "ALGO"   ,ch+1, &CH->ALGO , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_FM, "FB"   ,ch+1, &CH->FB , 1*sizeof(u8),REG_u8);

	/* slots */
	for(slot=0;slot<4;slot++) {
	    FM_SLOT *SLOT = &CH->SLOT[slot];
	    create_state_register(ST_YM2610_FM, "phasecount" ,(ch<<4)|slot, &SLOT->Cnt, 1*sizeof(u32),REG_u32);
	    create_state_register(ST_YM2610_FM, "Incr"      ,(ch<<4)|slot, &SLOT->Incr, 1*sizeof(u32),REG_u32);
	    create_state_register(ST_YM2610_FM, "ksr"      ,(ch<<4)|slot, &SLOT->ksr, 1*sizeof(u8),REG_u8);
	    create_state_register(ST_YM2610_FM, "evc"     ,(ch<<4)|slot, &SLOT->evc, 1*sizeof(s32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "eve"     ,(ch<<4)|slot, &SLOT->eve, 1*sizeof(s32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "evs"     ,(ch<<4)|slot, &SLOT->evs, 1*sizeof(s32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "evsa"     ,(ch<<4)|slot, &SLOT->evsa, 1*sizeof(s32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "evsd"     ,(ch<<4)|slot, &SLOT->evsd, 1*sizeof(s32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "evss"     ,(ch<<4)|slot, &SLOT->evss, 1*sizeof(s32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "evsr"     ,(ch<<4)|slot, &SLOT->evsr, 1*sizeof(s32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "TLL"     ,(ch<<4)|slot, &SLOT->TLL, 1*sizeof(s32),REG_INT32);
	    create_state_register(ST_YM2610_FM, "TL"     ,(ch<<4)|slot, &SLOT->TL, 1*sizeof(u32),REG_INT32);
//	    create_state_register(ST_YM2610_FM, "EG_NEXT"     ,(ch<<4)|slot, &fm_state[(ch<<4)|slot].eg_next, 1*sizeof(u8));
	}
    }
#endif
}
...
{
#if 0
	create_state_register(ST_YM2610_FM, "address"   ,1, &ST->address , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_FM, "IRQ"       ,1, &ST->irq     , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_FM, "IRQ MASK"  ,1, &ST->irqmask , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_FM, "status"    ,1, &ST->status  , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_FM, "mode"      ,1, &ST->mode    , 1*sizeof(u32),REG_u32);
	create_state_register(ST_YM2610_FM, "prescaler" ,1, &ST->prescaler_sel , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_FM, "freq latch",1, &ST->fn_h , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_FM, "TIMER A"   ,1, &ST->TA   ,1*sizeof(s32),REG_INT32);
	create_state_register(ST_YM2610_FM, "TIMER Acnt",1, &ST->TAC  ,1*sizeof(s32),REG_INT32);
	create_state_register(ST_YM2610_FM, "TIMER B"   ,1, &ST->TB   ,1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_FM, "TIMER Bcnt",1, &ST->TBC  ,1*sizeof(s32),REG_INT32);
...
static const u8 OPN_FKTABLE[16] =
    { 0, 0, 0, 0, 0, 0, 0, 1, 2, 3, 3, 3, 3, 3, 3, 3 };

#if FM_LFO_SUPPORT
/* OPN LFO waveform table */
static s32 OPN_LFO_wave[LFO_ENT];
#endif
...
static void OPNSetPris(FM_OPN * OPN, int pris, int TimerPris, int SSGpris)
{
	...
	OPN->FN_TABLE[i] =
	    (u32) ((double) i * OPN->ST.freqbase * FREQ_RATE *
		      (1 << 7) / 2);
    }
#if FM_LFO_SUPPORT
    /* LFO freq. table */
...
#undef FM_LF
	for (i = 0; i < 8; i++) {
	    OPN->LFO_FREQ[i] = (u32) (freq_table[i] * OPN->ST.freqbase);
	}
    }
#endif
...
static void OPNWriteReg(FM_OPN * OPN, int r, int v)
{
	...
	case 2:		/* 0xa8-0xaa : 3CH FNUM1 */
	    if (r < 0x100) {
		u32 fn = (((u32) (OPN->SL3.fn_h[c] & 7)) << 8) + v;
		u8 blk = OPN->SL3.fn_h[c] >> 3;
		/* make keyscale code */
...
(int) (5.9 / EG_STEP), (int) (11.8 / EG_STEP) };
		CH->pms =
		    (s32) ((1.5 / 1200.0) * pmd_table[v & 7] * PMS_RATE);
		/* b4-5 AMS */
		/* 0 , 1.4 , 5.9 , 11.8(dB) */
...
static void FMsave_state_adpcma(ADPCM_CH *adpcm)
{
	...
	create_state_register(ST_YM2610_ADPCMA, "flag"    ,ch+1, &adpcm->flag      , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_ADPCMA, "flag_mask"    ,ch+1, &adpcm->flagMask      , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_ADPCMA, "now_data"    ,ch+1, &adpcm->now_data  , 1*sizeof(u8),REG_u8);
	create_state_register(ST_YM2610_ADPCMA, "now_addr"    ,ch+1, &adpcm->now_addr  , 1*sizeof(u32),REG_u32);
	create_state_register(ST_YM2610_ADPCMA, "now_step"    ,ch+1, &adpcm->now_step  , 1*sizeof(u32),REG_u32);
	create_state_register(ST_YM2610_ADPCMA, "IL"   ,ch+1, &adpcm->IL , 1*sizeof(s32),REG_INT32);

	create_state_register(ST_YM2610_ADPCMA, "step"    ,ch+1, &adpcm->step  , 1*sizeof(u32),REG_u32);
	create_state_register(ST_YM2610_ADPCMA, "start"    ,ch+1, &adpcm->start  , 1*sizeof(u32),REG_u32);
	create_state_register(ST_YM2610_ADPCMA, "end"    ,ch+1, &adpcm->end  , 1*sizeof(u32),REG_u32);

	create_state_register(ST_YM2610_ADPCMA, "adpcmx"   ,ch+1, &adpcm->adpcmx , 1*sizeof(s32),REG_INT32);
	create_state_register(ST_YM2610_ADPCMA, "adpcmd"  ,ch+1, &adpcm->adpcmd, 1*sizeof(s32),REG_INT32);
	create_state_register(ST_YM2610_ADPCMA, "adpcml"   ,ch+1, &adpcm->adpcml , 1*sizeof(s32),REG_INT32);
	create_state_register(ST_YM2610_ADPCMA, "volume"   ,ch+1, &adpcm->volume , 1*sizeof(s32),REG_INT32);
    }
...
/*
    for (i = 0; i < 0; i++) {
	aybuf[i] = alloca(sizeof(s16) * 16384);
    }
*/
    /* setup DELTA-T unit */
...
//    create_state_register(ST_YM2610, "ym2610"   ,1, F2610   , sizeof(YM2610));
    create_state_register(ST_YM2610, "out_ch"   ,1, out_ch   , 4*sizeof(s32),REG_INT32);
    create_state_register(ST_YM2610, "pg_in1"   ,1, &pg_in1   , 1*sizeof(s32),REG_INT32);
    create_state_register(ST_YM2610, "pg_in2"   ,1, &pg_in2   , 1*sizeof(s32),REG_INT32);
    create_state_register(ST_YM2610, "pg_in3"   ,1, &pg_in3   , 1*sizeof(s32),REG_INT32);
    create_state_register(ST_YM2610, "pg_in4"   ,1, &pg_in4   , 1*sizeof(s32),REG_INT32);


// #if 1
    create_state_register(ST_YM2610, "regs"   ,1, F2610->REGS   , 512*sizeof(u8),REG_u8);
    FMsave_state_st(&FM2610.OPN.ST);
    FMsave_state_channel(FM2610.CH,6);
    /* 3slots */
    create_state_register(ST_YM2610, "slot3fc" ,1, F2610->OPN.SL3.fc , 3*sizeof(u32),REG_u32);
    create_state_register(ST_YM2610, "slot3fh" ,1, &F2610->OPN.SL3.fn_h , 1*sizeof(u8),REG_u8);
    create_state_register(ST_YM2610, "slot3kc" ,1, F2610->OPN.SL3.kcode , 3*sizeof(u8),REG_u8);
    /* address register1 */
    create_state_register(ST_YM2610, "address1" ,1, &F2610->address1, 1*sizeof(s32),REG_INT32);
    create_state_register(ST_YM2610, "arrivedFlag",1, &F2610->adpcm_arrivedEndAddress , 1*sizeof(u8),REG_u8);
    /* rythm(ADPCMA) */
	       ...
	       FM_TIMERHANDLER TimerHandler, FM_IRQHANDLER IRQHandler)
{
    ...
    /* ADPCM */
    FM2610.pcmbuf = (u8 *) (pcmroma);
    FM2610.pcm_size = pcmsizea;
    /* DELTA-T */
    FM2610.deltaT.memory = (u8 *) (pcmromb);
