 ...
 */
static unsigned char l3_main_data[MAD_BUFFER_MDLEN];
static mad_fixed_t l3_overlap[2][32][18];
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  if (stream->main_data == 0) {
  	//debug_log("malloc");
    stream->main_data = l3_main_data;//malloc(MAD_BUFFER_MDLEN);
    if (stream->main_data == 0) {
  ...
  if (frame->overlap == 0) {
  	//debug_log("malloc2");
    frame->overlap = l3_overlap; //malloc(2 * 32 * 18* sizeof(mad_fixed_t));//calloc(2 * 32 * 18, sizeof(mad_fixed_t));
    if (frame->overlap == 0) {
    	debug_log("ko");
      stream->error = MAD_ERROR_NOMEM;
      return -1;
