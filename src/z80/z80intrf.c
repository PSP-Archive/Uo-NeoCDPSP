...
static u32 cpu_readmem8(unsigned int address)
{
	return (subcpu_memspace[address]);
}

#if CZ80_USE_WORD_HANDLER
static u32 cpu_readmem16(unsigned int address)
{
  // return cpu_readmem8(address) | (cpu_readmem8(address + 1) << 8);
  return (*(u16 *)(&subcpu_memspace[address]));
/*        char buff[10];
	sprintf(buff,"%x",address);
	alert("read word",buff);
	return (*(unsigned short *)(&subcpu_memspace[address])); */
}
#endif
...
{
  // if (address >= 0xf800)
	subcpu_memspace[address]=data;
}

#if CZ80_USE_WORD_HANDLER
static void cpu_writemem16(unsigned int address, unsigned int data)
{
  (*(unsigned short *)(&subcpu_memspace[address])) = data;
  // if (address >= 0xf800) {
        /* char buff[10];
	sprintf(buff,"%x",address);
	char buff2[10];
	sprintf(buff2,"%x",data);
	alert(buff,buff2);
	(*(unsigned short *)(&subcpu_memspace[address])) = data; */
...
void cpu_z80_iret_callback(void)
{
  if (ExitOnEI) {
    ExitOnEI = 0;
    Cz80_struc.CycleIO = 0;
  }
}
...
*/
	default:
		//sprintf(z80_str,"Unimplemented Z80 Write Port: %x data: %x\n",PortNo&0xff,data);
		//debug_log(z80_str);
		break;
	}
}
...
*/
	default:
		//sprintf(z80_str,"Unimplemented Z80 Read Port: %x\n",PortNo&0xff);
		//debug_log(z80_str);
		break;
	};
	return 0;
...
void z80_init(void)
{
	...
	Cz80_Set_ReadB(&Cz80_struc,&cpu_readmem8);
	...
	Cz80_Set_RETI_Callback(&Cz80_struc,cpu_z80_iret_callback);
