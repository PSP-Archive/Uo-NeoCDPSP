...
int save_bmp(const char *file,int width,int height,int bit,void *bits,int pitch)
{
	...
	fwrite(&bfType,1,sizeof(bfType),fd);
	fwrite(&bf,1,sizeof(bf),fd);
	fwrite(&bi,1,sizeof(bi),fd);

	short *src = (short*)bits + pitch * height;
	char *temp_buf=(char*)malloc(width*3);
	int y,x;
	for(y=height-1;y>=0;--y) {
		src -= pitch;
		for (x=0;x<width;x++) {
			temp_buf[x*3+0]=((src[x]>>10)&31)<<3;
			temp_buf[x*3+1]=((src[x]>>5)&31)<<3;
			temp_buf[x*3+2]=((src[x]>>0)&31)<<3;
		}
		fwrite(temp_buf,1,width*3,fd);
	}
	free(temp_buf);
	fclose(fd);
	return 0;
...
static void basic_image_put(u16 *dst,IMAGE *img, int fade) {
	unsigned char* src = img->pixels;
	unsigned short pal[256];
	s32 r,g,b;
	int i;
	for(i=0;i<img->n_palette;i++){
	  r=(s32)(img->palette[i].r)-fade;if (r<0) r=0;
	  g=(s32)(img->palette[i].g)-fade;if (g<0) g=0;
	  b=(s32)(img->palette[i].b)-fade;if (b<0) b=0;
		pal[i] = RGB(r,g,b);
}
	int x,y;
...
{
  u16 *dst = (u16*)pgGetVramAddr(x0,y0);
  basic_image_put(dst,img,fade);
}
